...
features_output = f'../recordings/open-earable-ExG/AppVersion4_BLE/{participant_id}/{recording_date}/{participant_id}_{recording_date}_features_V2.csv'
...
def compute_cwt_features(data, scales, wavelet):
    ...
    for i, band in enumerate(BANDS.keys()):
        coeffs = cwt_matrix[i, :]
        features[f"cwt_{band}_variance"] = np.var(coeffs)
        features[f"cwt_{band}_skewness"] = pd.Series(coeffs).skew()
        features[f"cwt_{band}_kurtosis"] = pd.Series(coeffs).kurtosis()
